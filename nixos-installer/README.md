# Nix Environment Setup for Host: Saika
...
```bash
# NOTE: `cat saika.md | grep part-1 > part-1.sh` to generate this script
...
```bash
# NOTE: `cat saika.md | grep create-btrfs > btrfs.sh` to generate this script
...
# NOTE: `cat saika.md | grep mount-1 > mount-1.sh` to generate this script
...
btrfs filesystem mkswapfile --size 48g --uuid clear /mnt/swap/swapfile  # mount-1
...
nixos-install --root /mnt --flake .#saika --no-root-password --show-trace --verbose # install-1

# if you want to use a cache mirror, run this command instead
# replace the mirror url with your own
nixos-install --root /mnt --flake .#saika --no-root-password --show-trace --verbose --option substituters "https://mirrors.tuna.tsinghua.edu.cn/nix-channels/store https://mirror.sjtu.edu.cn/nix-channels/store https://mirrors.ustc.edu.cn/nix-channels/store https://cache.nixos.org/" # install-2
